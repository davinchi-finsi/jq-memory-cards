(function ($) {
    $.widget("ui.memoryGame", {
        ...
        _generateCards: function () {
            ...
            if (!this.autoPair) {
                // group by id
                const pairs = this._cards.reduce(
                    (acc, item) => {
                ...
                if (errors.length > 0) {
                    throw new Error(
                        "When the option 'autoPair' = false, all the items (cards) must be specified, 2 items for each id, but an incorrect number of items has been found:\n"+
                        errors.join("\n")
                    );
                }
            }
